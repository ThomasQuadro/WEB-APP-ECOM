  ...
  return (
    <div className="flex flex-col items-center justify-center py-28 gap-5 text-g-muted">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-2">
        <Link to="/boutique"      className="btn-primary text-sm px-5 py-2.5"><ShoppingCart size={16} /> Explorer la boutique</Link>
        <Link to="/configurateur" className="btn-secondary text-sm px-5 py-2.5">Créer sur-mesure</Link>
      </div>
    </div>
  )
}
  ...
  return (
    <tr className="border-b border-g-border last:border-0">
      <td className="py-5 px-4">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            {item.variant && <p className="text-xs text-g-muted mt-0.5 max-w-xs truncate">{item.variant}</p>}
            <p className="text-g-blue-l font-semibold text-sm mt-1">{item.price.toLocaleString('fr-FR')} € / unité</p>
          </div>
        </div>
      </td>
      <td className="py-5 px-4">
      ...
      <td className="py-5 px-4 text-right">
        <p className="font-bold text-g-text whitespace-nowrap">{(item.price * item.quantity).toLocaleString('fr-FR')} €</p>
      </td>
      <td className="py-5 px-4 text-right">
        <button onClick={onRemove}
          className="p-2 rounded-lg text-g-muted hover:text-red-400 hover:bg-red-400/10 transition-all duration-150">
          <Trash2 size={16} />
        </button>
      </td>
    </tr>
  ...
  return (
    <div className="card p-6 space-y-4 sticky top-24">
      ...
      <div className="space-y-3 text-sm">
        ...
        {shipping > 0 && <p className="text-xs text-g-muted bg-g-card2 rounded-lg p-2">Livraison offerte dès 1 000 € d'achat</p>}
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Code promo" className="input pl-9 py-2 text-sm" />
        </div>
      ...
      <div className="space-y-2 pt-2">
        {[[ShieldCheck, 'Paiement 100% sécurisé'], [Truck, 'Expédition sous 48h']].map(([Icon, text]) => (
          <div key={text} className="flex items-center gap-2 text-xs text-g-muted">
            <Icon size={13} className="text-g-green" /> {text}
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="border-b border-g-border bg-g-card/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            {totalItems > 0 && <span className="badge-blue">{totalItems} article{totalItems > 1 ? 's' : ''}</span>}
          </div>
          {!user && totalItems > 0 && (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-g-border">
                      <th className="text-left text-xs font-bold text-g-muted uppercase tracking-widest py-3 px-4">Produit</th>
                      <th className="text-left text-xs font-bold text-g-muted uppercase tracking-widest py-3 px-4">Qté</th>
                      <th className="text-right text-xs font-bold text-g-muted uppercase tracking-widest py-3 px-4">Sous-total</th>
                      <th className="py-3 px-4" />
                    </tr>
                  </thead>
                  <tbody>
                    {items.map(item => (
                      <CartRow key={item._key} item={item}
